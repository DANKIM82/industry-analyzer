        ...
        for work in scopes:
            ...
            for name in account_names:
                ...
                for _, row in work[mask].iterrows():
                    ...
                    if v is None:
                        v = conv(row.get('thstrm_add_amount'))
                    if v is not None:
                        return v
    return 0
...
def parse_common(df, revenue_names=('매출액', '영업수익', '수익(매출액)')):
    ...
    return {
        ...
        'op_cashflow': op_cf,
        'eps': eps,
        'roe': safe_div(net_income, equity, pct=True),
    }
...
def parse_defense(df, company):
    """방산: 공통 재무지표 + 수주/수출 지표.
    defense.html FIELD_MAP 기대 필드:
      domestic_order, export_order, export_ratio, backlog
    DART 표준계정에 수주 관련 항목이 없는 경우가 많으므로,
    가능한 계정명을 순서대로 탐색하고 없으면 0 반환."""
    common = parse_common(df)

    # 수주잔고 합계
    backlog = extract_value(df, [
        '수주잔고', '수주잔액', '미이행수주잔액', '수주계약잔액',
        '장기미청구채권', '계약자산',
    ])

    # 수출 수주
    export_order = extract_value(df, [
        '수출수주잔고', '수출수주잔액', '수출수주', '해외수주잔고',
        '해외수주잔액', '해외수주', '해외매출채권',
        '수출계약자산', '해외계약자산',
    ])

    # 국내 수주
    if backlog and export_order:
        domestic_order = round(max(backlog - export_order, 0), 1)
    elif backlog and not export_order:
        domestic_order = backlog
    else:
        domestic_order = 0

    # 수출비중 — 수출매출액 / 매출액
    export_sales = extract_value(df, [
        '수출매출액', '해외매출액', '해외매출',
        '수출부문매출', '방산수출매출', '수출',
    ])
    revenue = common.get('revenue', 0)
    if export_sales and revenue:
        export_ratio = safe_div(export_sales, revenue, pct=True)
    elif export_order and backlog:
        export_ratio = safe_div(export_order, backlog, pct=True)
    else:
        export_ratio = 0
...
def parse_kfood(df, company):
    """K-푸드: 공통 재무지표 + 수출 지표.
    kfood.html 기대 필드:
      revenue, op_income, op_margin, net_income, net_margin,
      op_cashflow, eps, roe,
      export_revenue, export_ratio, north_america, asia, europe
    DART 주석/세그먼트에서 해외 매출 계정을 탐색하고 없으면 0 반환."""
    common = parse_common(df)
    revenue = common.get('revenue', 0)

    # 수출 매출 전체
    export_revenue = extract_value(df, [
        '수출매출액', '해외매출액', '해외매출', '수출액',
        '수출', '해외부문매출', '해외사업부문매출',
        '해외사업매출', '국외매출',
    ])
    ...
    asia = extract_value(df, [
        '아시아', '아시아매출', '아시아태평양', '중국',
        '동남아', '아시아지역', '아태',
    ])
    europe = extract_value(df, [
        '유럽', '유럽매출', 'Europe', '유럽지역',
    ])

    # 지역 합이 수출합보다 크면 지역합 사용
    region_sum = round(north_america + asia + europe, 1)
    if region_sum > export_revenue:
        export_revenue = region_sum
...
def parse_kbeauty(df, company):
    """K-뷰티: 공통 재무지표 + 해외 지표.
    kbeauty.html MONEY_FIELDS: revenue, op_income, net_income, op_cashflow,
                               overseas_revenue, north_america, china
    기대 필드 추가: overseas_ratio
    DART 세그먼트/주석에서 해외 매출 계정을 탐색하고 없으면 0 반환."""
    common = parse_common(df)
    revenue = common.get('revenue', 0)
    ...
    # 지역 합이 해외합보다 크면 지역합 사용
    region_sum = round(china + north_america, 1)
    if region_sum > overseas_revenue:
        overseas_revenue = region_sum
